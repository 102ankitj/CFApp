...
class RegistrationForm(
    forms.Form):  #Creating an object named RegistrationForm from imported forms to contain specified attributes
    username = forms.CharField()  #Creates a character field and stores the input in username
    password = forms.CharField(
        widget=forms.PasswordInput)  #Creates a Password field as a widget to contain pre-defined syntax validated values
    password2 = forms.CharField(
        widget=forms.PasswordInput)  #Adding another field of password using same specifications for password confirmation
    ...
    def clean(self):  #Defining a function to clean the current class
        ...
        password2 = self.cleaned_data.get(
            'password2')  #Clean the input value in password2 field and store it in password2

        if not (password == password2):  #If values in password and password2 are not similar, do:
            self.errors['password'] = ErrorList(
                [u"Passwords are not the same"])  #Generate an error in unicode using ErrorList to alert the user

        try:
            if not User.objects.get(
                    username=username) == None:  #If the value in username is already present in previous User object usernames, do:
                self.errors['username'] = ErrorList([
                    u"Too slow; This name already taken"])  #Generate an error in unicode using ErrorList to alert the user
        except:
...
class AddCakeForm(forms.Form):  #Creating an object named AddCakeForm from imported forms to contain specified fields
    ...
    quantity = forms.IntegerField(
        min_value=1)  #Creating an integer field to specify the required quantity of the selected cake and setting the minimum quantity to 1

    quantity.widget.attrs.update(
        {'class': 'form-control'})  #Creating a widget to input the quantity field on the webform
...
class BasketForm(forms.Form):  #Creating an object named BasketForm from imported forms to contain specified attributes
    ...
    address.widget.attrs.update(
        {'class': 'form-control'})  #Creating a widget to input the delivery address on the rendered webform