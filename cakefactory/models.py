...
class Cake(models.Model, PriceableMixin):					#Creating a Model class object named Cake to contain specified attributes
    ...
    weight = models.IntegerField(blank=True, null=True)		#Set the weight of the cake in whole numbers. This field can be left empty
    ingredients = models.ManyToManyField('Ingredient')		#Select the ingredients of the cake from Ingredients model. More than one ingredient can be chosen.
...
class OrderItem(models.Model):								#Creating a Model class object OrderItem
    related_order = models.ForeignKey('Order')				#Set the order ID from Order model while the customer is shopping to store their selected cakes
	...
	#Funtion to set the return string from this model
    def __str__(self):
        return "{0}: {1}".format(self.product, self.quantity)	#Return the ID for selected cake and desired quantity when this model is used
...
class Order(models.Model):									#Creating a Model class object named Order
    ...
    address = models.CharField(max_length=255, null=True)	#Input the address of the customer using a maximum of 255 characters. The field is empty when the page is rendered
    customer = models.ForeignKey(User)						#Get the customer details from User model
    current = models.BooleanField(default=True)				#Check if the customer is still shopping. True if still shopping, False if customer is checking out
